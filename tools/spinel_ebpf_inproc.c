 ...
 * earlier `build/spinel --emit-ir` + text-codegen chain; tools/stage2_verify.sh
 * diffs its output against the committed goldens to prove byte-identity over
 * every fixture.
 *
 ...
 *             against `build/spinel --emit-ir` (proves Patch A can be removed). */
