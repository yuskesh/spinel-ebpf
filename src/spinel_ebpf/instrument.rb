...
#
# `--instrument` auto-instrumentation, driven by the compiler's symbol map.
#
# spinel is the compiler, so it knows every method and the exact C symbol it
# lowers to. Upstream `--emit-symbol-map` (f5e1a22a) dumps that mapping as JSON
...
#      (without it, non-recursive leaves are inlined away and lose their symbol),
#   3. generates an agent that puts a uprobe/uretprobe on the symbol and records
#      per-method rate and log2 latency in the kernel's keyed histogram.
#
...
module SpinelEbpf
  module Instrument
    ...
    #   uretprobe: hist_observe_by(idx, lat_end(...))   -- per-method log2 histogram
    ...
    # One-shot mode: observe for wait_seconds, then dump per-method count/p50/p99/
    ...
    def generate_metrics_agent(records, target_bin:, port: 9100, depth_collapse: false)
      <<~RUBY
        # GENERATED by spinel-ebpf --instrument (the Prometheus /metrics server). Do not edit.
    ...
    # OTLP push helpers — register each instrumented method (idx = keyed-hist
    # key) then push the accumulated kernel hist as OTLP/HTTP+protobuf. The FFI module
    # and __spnl_otlp_export are native (module FFI + __spnl_ prefix), like the /metrics
    # helpers. spnl_otlp_add_method/set_service resolve to otlp_agent.c; spnl_otlp_push
    # to the glue shim (reads the loaded bpf_object).
    # Generates the register-and-push code shared by metrics and traces. The signal,
    # :metrics or :traces, only selects the push FFI (spnl_otlp_push or
    # spnl_otlp_trace_push) and the label used in the output; the generated
    # __spnl_otlp_export is named the same either way.
    def otlp_export_defs(records, endpoint, signal)
      push = signal == :traces ? "spnl_otlp_trace_push" : "spnl_otlp_push"
      adds = records.map { |r|
        %(  SpnlOtlp.spnl_otlp_add_method("#{r[:ruby]}", "#{r[:file]}", #{r[:line] || 0}, #{r[:idx]}))
      }.join("\n")
      <<~RUBY
        module SpnlOtlp
          ffi_func :spnl_otlp_set_service, [:str, :str],              :int
          ffi_func :spnl_otlp_add_method,  [:str, :str, :long, :long], :int
          ffi_func :#{push}, [:str], :int
        end

        def __spnl_otlp_export
          SpnlOtlp.spnl_otlp_set_service("spinel-ebpf-instrument", "0")
        #{adds}
          st = SpnlOtlp.#{push}("#{endpoint}")
          puts "[instrument] OTLP #{signal} push -> #{endpoint} HTTP " + st.to_s
        end
      RUBY
    end

    # trace probe bodies — emit enter/exit span events via the existing
    # spnl_emit4 ringbuf (kind 0/1, idx, ktime_ns, tid). No new eBPF codegen.
    def trace_probe_defs(records)
      ups = records.map { |r| "def uprobe__#{r[:c]}\n  spnl_emit4(0, #{r[:idx]}, ktime_ns, tid)\nend" }.join("\n\n")
      urs = records.map { |r| "def uretprobe__#{r[:c]}\n  spnl_emit4(1, #{r[:idx]}, ktime_ns, tid)\nend" }.join("\n\n")
      "#{ups}\n\n#{urs}"
    end

    # single self-attaching binary. The workload (target_source,
    # verbatim) and the agent live in one unit; the uprobes self-attach (/proc/self/exe
    # at own pid). The workload runs in this process (its sp_<method> calls fire the
    # self-uprobe -> kernel hist), then the agent either serves /metrics (pull) or,
    # with otlp_endpoint, pushes the accumulated hist once as OTLP and exits.
    def generate_self_agent(records, target_source, port: 9100, depth_collapse: false,
                            otlp_endpoint: nil, otlp_traces: false)
      if otlp_endpoint
        # One branch covers both signals: it selects the probe (histogram or emit4)
        # and the push function from the signal.
        signal = otlp_traces ? :traces : :metrics
        probes = otlp_traces ? trace_probe_defs(records)
                             : probe_defs(records, depth_collapse: depth_collapse)
        flag   = otlp_traces ? " --instrument-otlp-traces" : ""
        kind   = otlp_traces ? "call tree as OTLP traces (spans)" : "per-method RED as OTLP metrics"
        <<~RUBY
          # GENERATED by spinel-ebpf --instrument-self --instrument-otlp#{flag}. Do not edit.
          # Single binary: workload + self-attaching uprobe agent that pushes the
          # #{kind} to #{otlp_endpoint} after the workload runs.
          use_plugin :ebpf

          #{probes}

          #{otlp_export_defs(records, otlp_endpoint, signal)}
          # === workload (your program, unchanged) — self-uprobe fires/emits on its calls ===
          #{target_source}
          # === workload done -> push as OTLP #{signal} ===
          __spnl_otlp_export
        RUBY
      else
        <<~RUBY
