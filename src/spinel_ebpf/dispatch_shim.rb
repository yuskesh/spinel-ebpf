...
#
# spinel-ebpf -- the dispatch shim emitter.
#
...
#
# The boundary ABI (see BOUNDARY_* below):
...
module SpinelEbpf
  module DispatchShim
    # the spinel<->eBPF boundary ABI for transparent
    # dispatch as a first-class, validated contract. A native call to a
    # dispatched :ebpf method crosses as:
    ...
    #     return as void — matching spinel's extern decl (Patch 1, the canonical
    ...
    # BoundaryError. The boundary copies values; pointers never cross it.
    # Deferred (Phase 4 continuation): 64-bit return via ctx out-param,
    # Result/option tagged emit, borrow/own handle typing.
