...
// Source unit: 185_emit_dns_kfield_filter.rb
...
} u_185_emit_dns_kfield_filter_lost SEC(".maps");

static __always_inline void spnl_lost_inc(void)
{
    __u32 _z = 0;
    __u64 *_l = bpf_map_lookup_elem(&u_185_emit_dns_kfield_filter_lost, &_z);
    if (_l) *_l += 1;
}
...
struct u_185_emit_dns_kfield_filter_dns_event {
...
} u_185_emit_dns_kfield_filter_dns_events SEC(".maps");
...
const struct u_185_emit_dns_kfield_filter_dns_event *_spnl_rec_dns_event SEC(".spnl_records");
...
static __noinline __s64 kprobe__udp_sendmsg_inner(__s64 sk, __s64 msg, __s64 len)
{
    ...
    if (((__s64)BPF_CORE_READ((struct sock *)(unsigned long)(sk), __sk_common.skc_dport)) == 13568) {
        {
            struct u_185_emit_dns_kfield_filter_dns_event *_de2 = bpf_ringbuf_reserve(&u_185_emit_dns_kfield_filter_dns_events, sizeof(*_de2), 0);
            if (_de2) {
